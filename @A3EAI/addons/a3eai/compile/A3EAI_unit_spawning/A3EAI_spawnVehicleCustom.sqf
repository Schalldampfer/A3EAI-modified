...
if (_isAirVehicle) then {
	...
	if ((!isNull _vehicle) && {!isNull _unitGroup}) then {
		...
		if (A3EAI_debugLevel > 1) then {diag_log format ["A3EAI Debug: Custom AI helicopter crew group %1 is now active and patrolling.",_unitGroup];};
	};
} else {
	if ((!isNull _vehicle) && {!isNull _unitGroup}) then {
		A3EAI_curLandPatrols = A3EAI_curLandPatrols + 1;
		if (A3EAI_debugLevel > 1) then {diag_log format ["A3EAI Debug: Custom AI land vehicle crew group %1 is now active and patrolling.",_unitGroup];};
	};
};
